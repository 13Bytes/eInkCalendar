...
FONT_POPPINS_BOLD_P = ImageFont.truetype(
...
EVENT_NAME_FONT = ImageFont.truetype(
	os.path.join(FONT_DICT, 'Poppins-Bold.ttf'), 20)
EVENT_CALENDAR_FONT = ImageFont.truetype(
	os.path.join(FONT_DICT, 'Poppins-SemiBold.ttf'), 16)
LINE_WIDTH = 3
...
def render_content(draw_blk: TImageDraw, image_blk: TImage,	 draw_red: TImageDraw, image_red: TImage, height: int, width: int):
	...
	for event in event_list:
		...
		if last_event_day != event.start.date():
			...
			draw_red.text((PADDING_L, current_height), day_string, font=FONT_ROBOTO_P, fill=1, stroke_width=0, stroke_fill=0)
		...
		draw_blk.text((PADDING_L + summmary_padding, current_height), event.summary,
					  font=EVENT_NAME_FONT, fill=1)
		current_height += get_font_height(EVENT_NAME_FONT) * 1.1
		#Calendar Name
		draw_blk.text((PADDING_L + summmary_padding, current_height), event.calendar_name,
					  font=EVENT_CALENDAR_FONT, fill=1)
		current_height += get_font_height(EVENT_CALENDAR_FONT) * 1.1
